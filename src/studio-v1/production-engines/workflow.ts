...
export async function runStandaloneFamilyReviewEvidenceActivity(activity:{id:string;workflowRunId:string;workerId:string}) {
  ...
  try {
    ...
    const result=await executeStudioFamilyEngine({schema:"StudioFamilyEngineRequestV1",operation:"BUILD_INTERNAL_REVIEW_EVIDENCE",family:draft.family as "EXPLAINER"|"WHITEBOARD",authorityId:authority.authorityId,productionId:run.productionId,creativeStateArtifactId:creative.id,creativeStateArtifactHash:creative.contentHash,durationSeconds:Number(quote?.approvedDurationSeconds||draft.duration||60),aspectRatio:draft.aspectRatio,voicePreference:draft.voicePreference,outputDirectory,finalBoard,creativeCheckpoint:record(content.checkpoint),creativeDossier:record(content.dossier),referenceMedia,brandExecution});
