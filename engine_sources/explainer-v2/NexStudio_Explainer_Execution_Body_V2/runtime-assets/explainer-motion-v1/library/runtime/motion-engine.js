window.NexMotion = (() => {
  ...
  class MotionTimeline{
    ...
    seek(t){const previousNow=this.now;this.now=clamp(Number(t)||0,0,this.duration());this.base.seek(this.now);for(const u of this.custom){const raw=u.duration===0?(this.now>=u.start?1:0):clamp((this.now-u.start)/u.duration);u.fn(ease(u.easing,raw),raw,this.now,previousNow)}return this}
