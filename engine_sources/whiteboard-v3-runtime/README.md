...
- **One evolving board world — the giant-board journey (default).** Scenes live at distinct zones spread along a meandering path on one huge canvas; the camera pans and pulls back between them (~1.5 s bowed travels with neighboring ink visible at the frame edge) and finishes on a pull-out reveal of the real world the video traveled through. `cluster_travel` (0.62 s moves on one shared zone, mosaic reveal) remains available via plan `camera_variant` or `--variant`.
