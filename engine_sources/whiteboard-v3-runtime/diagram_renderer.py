...
def build_elements(plan: dict, ratio: str) -> tuple[list[dict], dict]:
    ...
    if flow and beats:
        ...
        for bi in range(len(beats)):
            ...
            if vert_cells:
                # distribute evenly across the whole safe zone — cells
                # spread top to bottom with real gaps, never bunched
                avail = z['h'] * 0.66
                top = -z['h'] * 0.30
                cell_h = avail * 0.62 / n
                gap = avail * 0.38 / max(n - 1, 1) if n > 1 else 0.0
                cells.append((0.0,
                              top + local * (cell_h + gap) + cell_h / 2
                              if n > 1 else top + avail / 2,
                              z['w'] * 0.90, cell_h))
            else:
                span = z['w'] * 0.86
                cw = span / n
