...
def _build(plan, ratio):
    ...
    for bi, beat in enumerate(beats):
        ...
        # figure anchor: bottom-left column when a speech bubble shares
        # the panel (bubble keeps the right column), else bottom-center
        fx = (rect[0] + rect[2]) / 2 + pw * 0.0
        if fig is not None:
            fx = rect[0] + pw * float(fig.get('x', 0.28 if bub else 0.5))
        fy = rect[3] - ph * 0.05
        fsize = ph * float((fig or {}).get('size',
                                          0.58 if bub else 0.72))
        ...
        if bub:
            ...
            pad = th_ * 0.9
            bw_max = pw * 0.52
            # shrink the hand-lettered size until the longest word
            # fits inside the bubble — a single long word cannot wrap
            while True:
                lines = _wrap(bub.get('text', ''), bw_max - pad * 2, th_)
                need = max((text_width(ln, th_) for ln in lines),
                           default=0.0)
                if need <= bw_max - pad * 2 or th_ < ph * 0.03:
                    break
                th_ *= 0.85
                pad = th_ * 0.9
            bw = max(th_ * 2.4, min(
                ...
                max((text_width(ln, th_) for ln in lines),
                    default=0.0) + pad * 2))
            bh_ = th_ * 1.30 * max(1, len(lines)) + pad * 1.4
            bcx = rect[0] + pw * float(bub.get('x', 0.70))
            bcx = max(rect[0] + bw / 2 + pw * 0.03,
                      min(bcx, rect[2] - bw / 2 - pw * 0.03))
            # right column beside the figure's head and shoulders
            bcy = rect[1] + ph * float(bub.get('y',
                                             0.48 if cap else 0.40))
        ...
        # narration caption box — small rect + text at the panel's
        # top-left; the font shrinks until the line fits the panel
        cap_groups = []
        if cap:
            ch_ = ph * 0.11
            ctext = cap.upper()
...
def _fig_frame(spec, facing):
    ...
    if not strip:
        return None
    if 'pose' not in spec:
        # pick the most readable freeze — the most slender silhouette
        # (standing poses are tall AND narrow; a crouch is tall too but
        # never slender, so aspect beats height as a proxy)
...
def render_comic_frame(plan: dict, ratio: str, t: float):
    ...
    bubble_layer = Image.new('RGBA', (vw, vh), (0, 0, 0, 0))
    tip = None
    ...
    for sec in flow['panels']:
        ...
        if p > 0 and sec['bub_groups']:
            ...
            t2 = _draw_strokes(bubble_layer, g[1], g[2], g[3], cam,
                               colors, ratio, p, g_seed + 2, zoom)
            if t2:
                tip = t2
    ...
    # bubble ink goes over the pasted figure — comic convention has
    # the speech bubble in front of the speaker
    frame = Image.alpha_composite(
        frame.convert('RGBA'), bubble_layer).convert('RGB')
